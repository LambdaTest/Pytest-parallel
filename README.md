# Run Pytest Tests in Parallel with Selenium on TestMu AI (Formerly LambdaTest)
...
With TestMu AI (Formerly LambdaTest), you can run your Pytest Selenium tests in parallel across multiple browsers and configurations simultaneously, dramatically cutting down total test execution time. This sample shows how to configure parallel test execution in Pytest on the TestMu AI cloud.
...
Ensure that the pytest-selenium package is installed:

```bash
pip install pytest-selenium
```

To run parallel tests using pytest, execute the below command in the terminal:

```bash
pytest -s -n=2 tests/lt_sample_todo.py
```

In the above command, `-s` disables output capture and `-n` is used to specify the number of parallel test executions.
